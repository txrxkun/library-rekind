...
// refresh otomatis setiap 30 detik untuk menghilangkan popup login
    ...
    public function getAttachments() {
        ...
        foreach ($this->record_detail['attachments'] as $attachment_d) {
          if ($attachment_d['mime_type'] == 'application/pdf') {
            $_output .= '<li class="attachment-pdf" style="list-style-image: url(images/labels/ebooks.png)" itemscope itemtype="http://schema.org/MediaObject">
            <a itemprop="name" property="name" class="openPopUp" title="'.$attachment_d['file_title'].'" href="./index.php?p=fstream&fid='.$attachment_d['file_id'].'&bid='.$attachment_d['biblio_id'].'" width="780" height="520">'.$attachment_d['file_title'].
            ...
            // $sql = mysqli_query($conn, "INSERT INTO ebook_log(member_id, biblio_id) VALUES('$a', '$biblioID')");
            // echo $a; 
            // echo "<br/>";
            // echo $attachment_d['biblio_id'];
           ...
           // }
            ...
            // $a = $_SESSION['mid'];
            // $b = $attachment_d['biblio_id'];

            // $_output .='<input type="hidden" value="'.$a.'" name="mid" id="mid">';
            // $_output .='<input type="hidden" value="'.$b.'" name="bid" id="bid">';

            // $_output .= "<script>$('.insert').click(function () {
            // const mid = $('#mid').val();
            // const bid = $('#bid').val();
            //       $.ajax({
            //         type: 'POST',
            //         url: 'http://localhost/akasia/insert_log.php',
            ...
            // $sql = "INSERT INTO ebook_log(member_id, biblio_id) VALUES ($a,$b)";
            // var_dump($conn);
            // print_r($conn);
