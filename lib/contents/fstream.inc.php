...
if ($file_q->num_rows > 0) {
  $file_loc_url = SWB.'index.php?p=fstream-pdf&fid='.$fileID.'&bid='.$biblioID;
    ...
    $dbs->query("INSERT INTO ebook_log(member_id, biblio_id) VALUES($a, $biblioID)");
    // mysqli_query($conn, "INSERT INTO ebook_log(member_id, biblio_id) VALUES($a, $biblioID)");
  ...
  if (file_exists($file_loc)) {
    ...
    if ($file_d['access_limit']) {
      if (utility::isMemberLogin()) {
        ...
        $referto = SWB.'index.php?p=newlogin&destination=index.php?p=fstream&fid='.$fileID.'&bid='.$biblioID;
